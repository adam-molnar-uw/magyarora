...
const I = {
  ...
  Stairs:    () => <Svg d="M3 21h4v-4h4v-4h4v-4h4V5h4" />,
  Bubble:    () => <Svg d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />,
  Mic:       () => <Svg d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3zM19 10v2a7 7 0 0 1-14 0v-2M12 19v3M8 22h8" />,
  Lightbulb: () => <Svg d="M9 18h6M10 22h4M12 2a7 7 0 0 0-4 12.7c.7.7 1 1.5 1 2.3v1h6v-1c0-.8.3-1.6 1-2.3A7 7 0 0 0 12 2z" />,
  Eye:       () => <Svg d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8S1 12 1 12z"><circle cx="12" cy="12" r="3" /></Svg>,
};
...
// HOME (LESSONS LIST)
// ============================================================
function Home({ lessons, storeState, onPickLesson, onReview, onPractice }) {
  ...
  const practiceCount = lessons.filter((l) => l.practice).length;
  ...
  return (
    <div className="fade-up">
      ...
      <div className="stats">
        ...
        <button className="stat" onClick={onPractice} style={{ border: 'none', cursor: 'pointer' }}
          title="Practice activities">
          <div className="stat-num">{practiceCount}</div>
          <div className="stat-label">{practiceCount === 0 ? 'No practice' : 'Practice'}</div>
        </button>
...
// ============================================================
// PRACTICE — Hub + four activities
// ============================================================

function PracticeHub({ lessons, onPickActivity, onBack }) {
  // Aggregate available activities across all lessons that have practice content
  const lessonsWithPractice = lessons.filter((l) => l.practice);

  return (
    <div className="fade-up">
      <button className="back-btn" onClick={onBack}><I.ArrowL /> Home</button>

      <div className="hero">
        <div className="hero-eyebrow"><I.Target /> <span>Practice</span></div>
        <h1 className="hero-title">Gyakorlás</h1>
        <p className="hero-sub">Active practice activities — lighter than a quiz, deeper than reading.</p>
      </div>

      {lessonsWithPractice.length === 0 ? (
        <div className="empty">
          <div className="empty-title">No practice activities yet</div>
          <p>Practice activities will appear here as lessons get them. Right now Lesson 2C is the proof-of-concept.</p>
        </div>
      ) : (
        <div className="lesson-list">
          {lessonsWithPractice.map((l) => {
            const p = l.practice;
            return (
              <div key={l.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 20 }}>
                <div className="lesson-title" style={{ marginBottom: 4 }}>{l.title}</div>
                <div className="lesson-sub" style={{ marginBottom: 16 }}>{l.subtitle}</div>
                <div className="practice-grid">
                  {p.workedExamples?.length > 0 && (
                    <button className="practice-card" onClick={() => onPickActivity(l, 'worked')}>
                      <div className="practice-icon"><I.Stairs /></div>
                      <div className="practice-name">Worked Examples</div>
                      <div className="practice-count">{p.workedExamples.length} walkthrough{p.workedExamples.length === 1 ? '' : 's'}</div>
                    </button>
                  )}
                  {p.selfExplain?.length > 0 && (
                    <button className="practice-card" onClick={() => onPickActivity(l, 'explain')}>
                      <div className="practice-icon"><I.Lightbulb /></div>
                      <div className="practice-name">Explain It</div>
                      <div className="practice-count">{p.selfExplain.length} prompt{p.selfExplain.length === 1 ? '' : 's'}</div>
                    </button>
                  )}
                  {p.shadow?.length > 0 && (
                    <button className="practice-card" onClick={() => onPickActivity(l, 'shadow')}>
                      <div className="practice-icon"><I.Mic /></div>
                      <div className="practice-name">Shadow</div>
                      <div className="practice-count">{p.shadow.length} phrase{p.shadow.length === 1 ? '' : 's'}</div>
                    </button>
                  )}
                  {p.dialogue?.length > 0 && (
                    <button className="practice-card" onClick={() => onPickActivity(l, 'dialogue')}>
                      <div className="practice-icon"><I.Bubble /></div>
                      <div className="practice-name">Dialogue</div>
                      <div className="practice-count">{p.dialogue.length} conversation{p.dialogue.length === 1 ? '' : 's'}</div>
                    </button>
                  )}
                </div>
              </div>
...
// -------- Worked Examples --------
function WorkedExamplesView({ lesson, onBack }) {
  const examples = lesson.practice.workedExamples;
  const [exIdx, setExIdx] = useState(0);
  const [revealed, setRevealed] = useState(0); // how many steps revealed so far

  const ex = examples[exIdx];
  const allDone = revealed >= ex.steps.length;
  ...
  return (
    <div className="fade-up">
      <button className="back-btn" onClick={onBack}><I.ArrowL /> Practice</button>

      <div className="hero">
        <div className="hero-eyebrow"><I.Stairs /> <span>Worked Example · {exIdx + 1} of {examples.length}</span></div>
        <h1 className="hero-title" style={{ fontSize: 'clamp(28px, 5vw, 40px)' }}>{ex.goal}</h1>
      </div>
      ...
      <div className="quiz-box">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {ex.steps.map((step, i) => {
            ...
            return (
              <div key={i} className={`worked-step ${isRevealed ? 'done' : 'current'}`}>
                <div className="worked-prompt"><MD text={step.prompt} /></div>
                {isRevealed && (
                  <div className="worked-reveal fade-up"><MD text={step.reveal} /></div>
                )}
                {isCurrent && (
                  <button className="btn-text" onClick={() => setRevealed(revealed + 1)} style={{ marginTop: 8 }}>
                    <I.Eye /> Tap to reveal
                  </button>
                )}
              </div>
          ...
          {revealed < ex.steps.length && revealed === 0 && (
            <button className="btn-primary" style={{ alignSelf: 'flex-start' }} onClick={() => setRevealed(1)}>
              Start <I.Arrow />
            </button>
          )}
          ...
          {allDone && (
            <div className="quiz-actions">
              {exIdx + 1 < examples.length ? (
                <button className="btn-primary" onClick={next}>Next example <I.Arrow /></button>
              ) : (
                <button className="btn-primary" onClick={onBack}><I.Check /> Done</button>
              )}
            </div>
          )}
        </div>
      ...
      <div style={{ marginTop: 16, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {examples.map((_, i) => (
          <button key={i} className={`opt ${i === exIdx ? 'selected' : ''}`} style={{ padding: '6px 12px', fontSize: 12 }}
            onClick={() => { setExIdx(i); setRevealed(0); }}>
            #{i + 1}
          </button>
        ))}
      </div>
...
// -------- Self-Explain --------
function SelfExplainView({ lesson, onBack }) {
  const prompts = lesson.practice.selfExplain;
  const [idx, setIdx] = useState(0);
  const [mode, setMode] = useState('think'); // 'think' or 'type'
  const [text, setText] = useState('');
  ...
  const next = () => {
    setText('');
    setRevealed(false);
    if (idx + 1 >= prompts.length) onBack();
    else setIdx(idx + 1);
  };

  // Compute keyword hits if user typed
  const keywordsHit = mode === 'type' && text
    ? p.keywords.filter((kw) => text.toLowerCase().includes(kw.toLowerCase()))
    : [];
  ...
  return (
    <div className="fade-up">
      ...
      <div className="hero">
        <div className="hero-eyebrow"><I.Lightbulb /> <span>Explain It · {idx + 1} of {prompts.length}</span></div>
      </div>

      <div className="quiz-box">
        <div className="mode-toggle">
          <button className={`opt ${mode === 'think' ? 'selected' : ''}`} style={{ flex: 1, padding: 10, fontSize: 13 }}
            onClick={() => setMode('think')}>Just think it</button>
          <button className={`opt ${mode === 'type' ? 'selected' : ''}`} style={{ flex: 1, padding: 10, fontSize: 13 }}
            onClick={() => setMode('type')}>Type my answer</button>
        </div>

        <h3 className="quiz-q" style={{ marginTop: 22, marginBottom: 18 }}>{p.prompt}</h3>
        ...
        {mode === 'type' && (
          <div>
            <textarea
              ...
              style={{ minHeight: 100, fontSize: 15, lineHeight: 1.5, fontFamily: 'inherit', resize: 'vertical' }}
              autoCapitalize="sentences"
            />
            {revealed && p.keywords?.length > 0 && (
              <div style={{ marginTop: 10, fontSize: 13, color: 'var(--text-subtle)' }}>
                Keywords you mentioned: {keywordsHit.length > 0
                  ? keywordsHit.map((k, i) => <span key={i} className="pill" style={{ marginRight: 4, fontSize: 11 }}>✓ {k}</span>)
                  : <em>none yet — see model below</em>}
              </div>
            )}
          </div>
        )}
        ...
        {mode === 'think' && !revealed && (
          <p style={{ fontStyle: 'italic', color: 'var(--text-muted)', padding: '20px 0' }}>
            Form your answer in your head. When ready, tap below to compare with the model.
          </p>
        )}

        {revealed && (
          <div className="feedback ok fade-up" style={{ marginTop: 18 }}>
            <I.Lightbulb />
            <div>
              <div className="ttl">Model answer</div>
              <div className="exp"><MD text={p.modelAnswer} /></div>
            </div>
          </div>
        )}
        ...
        <div className="quiz-actions">
          {!revealed ? (
            <button className="btn-primary" onClick={() => setRevealed(true)}>
              <I.Eye /> Reveal model answer
            </button>
          ) : (
            <button className="btn-primary" onClick={next}>
              {idx + 1 >= prompts.length ? <>Done <I.Check /></> : <>Next <I.Arrow /></>}
            </button>
          )}
        </div>
      </div>
...
function ShadowView({ lesson, onBack }) {
  const phrases = lesson.practice.shadow;
  const [idx, setIdx] = useState(0);
  const [phase, setPhase] = useState('idle'); // idle | playing-1 | pause | playing-2 | done
  ...
  const playSequence = () => {
    if (timerRef.current) clearTimeout(timerRef.current);
    setPhase('playing-1');
    window.Store.speak(ph.text);
    // Estimate phrase duration: ~80ms per char + base 800ms
    const duration1 = Math.max(1500, ph.text.length * 80 + 800);
    timerRef.current = setTimeout(() => {
      setPhase('pause');
      timerRef.current = setTimeout(() => {
        setPhase('playing-2');
        window.Store.speak(ph.text);
        const duration2 = Math.max(1500, ph.text.length * 80 + 800);
        timerRef.current = setTimeout(() => setPhase('done'), duration2);
      }, 2500); // 2.5 second pause for user to repeat
    }, duration1);
  };
  ...
  const next = () => {
    ...
    if ('speechSynthesis' in window) speechSynthesis.cancel();
    setPhase('idle');
    if (idx + 1 >= phrases.length) onBack();
    else setIdx(idx + 1);
  };
  ...
  const phaseLabel = {
    idle: 'Tap to start',
    'playing-1': '🎙 Listen…',
    'pause': '🗣 Your turn — repeat aloud!',
    'playing-2': '🎙 Listen again…',
    done: '✓ Repeat or move on',
  }[phase];
  ...
  return (
    <div className="fade-up">
      ...
      <div className="hero">
        <div className="hero-eyebrow"><I.Mic /> <span>Shadow · {idx + 1} of {phrases.length}</span></div>
        <p className="hero-sub">Listen, repeat aloud during the pause, listen again to compare.</p>
      </div>

      <div className="quiz-box" style={{ textAlign: 'center' }}>
        <div className="font-display" style={{ fontSize: 32, fontWeight: 500, color: 'var(--accent)', margin: '20px 0 8px' }}>
          {ph.text}
        </div>
        <div style={{ fontStyle: 'italic', color: 'var(--text-subtle)', marginBottom: 28 }}>
          {ph.translation}
        </div>

        <div className={`shadow-phase phase-${phase}`} style={{ minHeight: 60, fontSize: 18, fontWeight: 500 }}>
          {phaseLabel}
        </div>
          ...
          {phase === 'idle' || phase === 'done' ? (
            <>
              <button className="btn-ghost" onClick={playSequence}>
                <I.Volume size={14} /> {phase === 'done' ? 'Again' : 'Start'}
              </button>
              {phase === 'done' && (
                <button className="btn-primary" onClick={next}>
                  {idx + 1 >= phrases.length ? <>Done <I.Check /></> : <>Next <I.Arrow /></>}
                </button>
              )}
            </>
          ) : (
            <button className="btn-ghost" disabled style={{ opacity: 0.5 }}>
              <I.Volume size={14} /> Playing…
            </button>
          )}
        </div>
      </div>

      <p style={{ fontSize: 12, color: 'var(--text-subtle)', textAlign: 'center', marginTop: 16 }}>
        Make sure your phone isn't on silent. Audio uses your installed Hungarian voice.
      </p>
    </div>
  );
}
...
// -------- Dialogue --------
function DialogueView({ lesson, onBack }) {
  const dialogues = lesson.practice.dialogue;
  const [dIdx, setDIdx] = useState(0);
  const [lineIdx, setLineIdx] = useState(0); // how many lines revealed so far
  const [userInput, setUserInput] = useState({}); // lineIdx → answer
  const [submitted, setSubmitted] = useState({}); // lineIdx → bool
  const [text, setText] = useState('');

  const dlg = dialogues[dIdx];
  ...
  const submitLine = (i, value) => {
    setUserInput({ ...userInput, [i]: value });
    setSubmitted({ ...submitted, [i]: true });
    // Auto-advance after 1 second so user reads feedback
    setTimeout(() => advance(), 1200);
  };

  const checkFill = (line, value) => {
    const clean = value.trim().toLowerCase().replace(/[.!?,]/g, '');
    return line.blanks[0].a.some((ans) => String(ans).toLowerCase().replace(/[.!?,]/g, '') === clean);
  };

  const allDone = lineIdx >= lines.length;

  // Auto-show first line
  useEffect(() => { if (lineIdx === 0) setLineIdx(1); }, [dIdx]);

  const nextDialogue = () => {
    if (dIdx + 1 >= dialogues.length) onBack();
    else { setDIdx(dIdx + 1); setLineIdx(0); setUserInput({}); setSubmitted({}); setText(''); }
  };
  ...
  return (
    <div className="fade-up">
      ...
      <div className="hero">
        <div className="hero-eyebrow"><I.Bubble /> <span>Dialogue · {dIdx + 1} of {dialogues.length}</span></div>
        <h1 className="hero-title" style={{ fontSize: 'clamp(26px, 4vw, 36px)' }}>{dlg.title}</h1>
        {dlg.context && <p className="hero-sub">{dlg.context}</p>}
      </div>
      ...
      <div className="dialogue-box">
        {lines.slice(0, lineIdx).map((line, i) => {
          ...
          if (!isUser) {
            return (
              <div key={i} className="dlg-line dlg-them fade-up">
                <div className="dlg-speaker">{line.speaker}</div>
                <div className="dlg-bubble dlg-them-bubble">
                  {line.text} <Speak text={line.text} />
                </div>
              </div>
            );
          }
          ...
          // User line
          if (!wasSubmitted && i === lineIdx - 1) {
            // active line — needs input
            if (line.options) {
              return (
                <div key={i} className="dlg-line dlg-you fade-up">
                  <div className="dlg-speaker">{line.speaker}</div>
                  <div className="opts" style={{ width: '100%' }}>
                    {line.options.map((opt, j) => (
                      <button key={j} className="opt" onClick={() => submitLine(i, j)}>
                        {opt}
                      </button>
                    ))}
                  </div>
                ...
                <div key={i} className="dlg-line dlg-you fade-up">
                  ...
                  <div style={{ width: '100%' }}>
                    <input
                      type="text" autoFocus value={text}
                      onChange={(e) => setText(e.target.value)}
                      onKeyDown={(e) => { if (e.key === 'Enter' && text.trim()) submitLine(i, text); }}
                      placeholder="Type your reply…"
                      className="fill-input"
                      autoCapitalize="sentences" autoCorrect="off" spellCheck="false"
                    />
                    {line.hint && <p className="hint">hint: {line.hint}</p>}
                    <button className="btn-primary" style={{ marginTop: 10 }}
                      onClick={() => submitLine(i, text)} disabled={!text.trim()}>
                      Send <I.Arrow />
                    </button>
                  </div>
                </div>
          ...
          // Submitted user line — show answer + feedback
          const userAns = userInput[i];
          let displayText, isCorrect, exp;
          if (line.options) {
            displayText = line.options[userAns];
            isCorrect = userAns === line.a;
            exp = line.exp;
          } else {
            displayText = userAns;
            isCorrect = checkFill(line, userAns);
            exp = line.exp;
          }
          ...
          return (
            <div key={i} className="dlg-line dlg-you fade-up">
              <div className="dlg-speaker">{line.speaker}</div>
              <div className={`dlg-bubble dlg-you-bubble ${isCorrect ? 'dlg-ok' : 'dlg-no'}`}>
                {displayText} <Speak text={displayText} />
                {!isCorrect && line.options && (
                  <div style={{ fontSize: 12, marginTop: 6, opacity: 0.85 }}>
                    Correct: {line.options[line.a]}
                  </div>
                )}
                {!isCorrect && line.blanks && (
                  <div style={{ fontSize: 12, marginTop: 6, opacity: 0.85 }}>
                    Correct: {line.blanks[0].a[0]}
                  </div>
                )}
                {exp && <div style={{ fontSize: 12, marginTop: 6, fontStyle: 'italic', opacity: 0.85 }}><MD text={exp} /></div>}
              </div>
            </div>
          );
        })}
        ...
        {allDone && (
          <div className="dlg-end fade-up">
            <p style={{ fontFamily: 'Fraunces, serif', fontSize: 22, color: 'var(--accent)', textAlign: 'center', margin: '24px 0 16px' }}>
              Vége. Szép munka!
            </p>
            <div className="quiz-actions" style={{ justifyContent: 'center' }}>
              {dIdx + 1 < dialogues.length ? (
                <button className="btn-primary" onClick={nextDialogue}>Next dialogue <I.Arrow /></button>
              ) : (
                <button className="btn-primary" onClick={onBack}><I.Check /> Done</button>
              )}
            </div>
...
function App() {
  ...
  } else if (view.name === 'practice') {
    content = <PracticeHub lessons={lessons}
      onBack={() => setView({ name: 'home' })}
      onPickActivity={(l, kind) => setView({ name: `practice-${kind}`, lesson: l })} />;
  } else if (view.name === 'practice-worked') {
    content = <WorkedExamplesView lesson={view.lesson} onBack={() => setView({ name: 'practice' })} />;
  } else if (view.name === 'practice-explain') {
    content = <SelfExplainView lesson={view.lesson} onBack={() => setView({ name: 'practice' })} />;
  } else if (view.name === 'practice-shadow') {
    content = <ShadowView lesson={view.lesson} onBack={() => setView({ name: 'practice' })} />;
  } else if (view.name === 'practice-dialogue') {
    content = <DialogueView lesson={view.lesson} onBack={() => setView({ name: 'practice' })} />;
    ...
    <div className="app-shell">
      <header className="header">
        ...
        <div className="header-actions">
          {onHome && (
            <>
              <button className="icon-btn" onClick={() => setView({ name: 'practice' })} aria-label="Practice">
                <I.Target />
              </button>
